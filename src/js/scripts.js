...
const yearsShowSelect = () => {
	...
	const selects = document.querySelector('.car-years__selects');
	selects.addEventListener('click', e => {
		const selectInput = e.target.closest('.car-years__input');
		if (!selectInput) {
			// console.log('вне инпута');
			return;
		}
		// console.log(e.target, selectInput, 'в инпут');
		const selector = selectInput.nextElementSibling; // блок со списком годов 'car-years__select'
...
document.addEventListener('click', function (e) {
	if (!e.target.closest('.car-years__input')) {
		// console.log(e.target, selectInput);
		document.querySelectorAll('.car-years__input').forEach(selectInput => {
