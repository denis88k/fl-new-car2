...
window.addEventListener('scroll', () => {
	...
	if (window.scrollY > TopChatConsultant * 1.03) {
		...
		consultSticky.classList.contains('show') && removeClass(consultSticky, 'show');
	}
});
		...
		selectInput.addEventListener('click', () => {
			...
			options.forEach(option => {
				option.onclick = e => {
					...
					// console.dir(e.target.parentElement.classList.contains('select-from'), 'e.target');

					if (e.target.parentElement.classList.contains('select-from')) {
						selectFrom = Number(e.target.dataset.value);
					}
					if (e.target.parentElement.classList.contains('select-to')) {
						selectTo = Number(e.target.dataset.value);
					}
					// в промежутке
					// const isBetween = checkboxBlock => Number(checkboxBlock.dataset.from) >= selectFrom && Number(checkboxBlock.dataset.to) <= selectTo;
					// // если конечный год в инпуте <=конечному году в checkboxBlock
					// const isEndStart = checkboxBlock => Number(checkboxBlock.dataset.from) <= selectTo;
					// // если начальный год в инпуте <=конечному году в checkboxBlock
					// const isStartEnd = checkboxBlock => Number(checkboxBlock.dataset.to) <= selectFrom;
					const isBetween = checkboxBlock =>
						(selectFrom <= Number(checkboxBlock.dataset.from) && Number(checkboxBlock.dataset.from) <= selectTo) ||
						(selectTo >= Number(checkboxBlock.dataset.to) && Number(checkboxBlock.dataset.to) >= selectFrom);
