...
const mileageInput = () => {
	...
	mileageInputs.addEventListener('input', e => {
		...
		// e.target.value = formatter.format(inputValue);
		// e.target.value = inputValue.toLocaleString('ru');
		e.target.value = e.target.value.replace(/\D/g, '').replace(/(\d)(?=(\d{3})+$)/g, '$1 ');
		// console.log(formatter.format(inputValue), 'fff');
		// console.log(inputValue.toLocaleString('ru'), 'fff');
